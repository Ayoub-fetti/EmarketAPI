...
const AdminStatsDashboard = memo(({ stats, monthlyRevenue, topProducts, recentOrders }) => {
  const statCards = [
    {
      label: "Total Revenue",
      value: currencyFormatter.format(stats.totalRevenue),
      sub: `Average order: ${currencyFormatter.format(stats.averageOrderValue)}`,
      icon: FaDollarSign,
    ...
    <div className="space-y-4 sm:space-y-6">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-6 shadow-md hover:shadow-lg transition-shadow duration-300 lg:col-span-2">
          <div className="flex items-center gap-2 sm:gap-3 mb-4">
            ...
            <div className="min-w-0">
              <h3 className="text-base sm:text-lg font-bold text-gray-900">Monthly Revenue</h3>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-6 shadow-md hover:shadow-lg transition-shadow duration-300">
          <div className="flex items-center gap-2 sm:gap-3 mb-4">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs sm:text-sm text-gray-500">Ranking by quantity sold.</p>
            </div>
          </div>
            ...
            <div className="h-64 sm:h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topProducts} layout="vertical">
                  ...
                  <Bar dataKey="quantity" fill="#ea580c" name="Quantity" radius={[0, 8, 8, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-4 sm:p-6 shadow-md hover:shadow-lg transition-shadow duration-300">
        <div className="mb-4 sm:mb-6">
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-1">The 6 most recent orders.</p>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200 text-xs sm:text-sm">
              ...
              <tbody className="divide-y divide-gray-100 bg-white">
                {recentOrders.map((order, index) => {
                  const orderCode = order._id ? `#${String(order._id).slice(-6)}` : "—";
                  const customer =
